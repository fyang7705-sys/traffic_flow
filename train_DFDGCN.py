...
from model.DFDGCN import DFDGCN, DFDGCNConfig
    ...
    for input_len in [12, 24, 48, 96]:
        for output_len in [12, 24, 48, 96]:
            ...
            BasicTSLauncher.launch_training(BasicTSForecastingConfig(
                model=DFDGCN, model_config=model_config, scaler=GraphScaler, norm_each_channel=True,
                dataset_name="traffic_flow", dataset_type=HybridPIBTDataset, data_file_path="data/den520d_lifelong/robot1000",
                use_timestamps=False, input_len=input_len, output_len=output_len, gpus="0",
                callbacks=[EarlyStopping(), GradientClipping(1.0)], seed=233, num_epochs=100, batch_size=32,
